...
const verifLinkValidate = Joi.object({    
   password : Joi.string().min(6).required() ,  
   email : Joi.string().min(6).required()   
})


 async function verifLink(req,res){
  ...
  try {
    ...
    if(user) {
      ...
      res.status(200).json({message:"verified ",email:user.email})   
    }
  ...
  // const {name,email,password} = req.body ;
    ...
    //     const { error ,details } = await verifLinkValidate.validateAsync(req.body)
    //      if (error) {
    //       if (details.length > 0) {
